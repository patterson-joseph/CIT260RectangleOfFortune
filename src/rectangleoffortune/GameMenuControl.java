...
public class GameMenuControl  {
    ...
    public void spin(Game game) {
        ...
        if(count > 0){
            game.playerList[game.currentPlayerNumberTurn].playerBank += count*game.spinner.spinnerValues[game.spinner.spinnerLocation];
            displayBorder();
            System.out.println("\tThere are " + count + " " + guessedLetter + "s. Your bank total is now: $" + game.playerList[game.currentPlayerNumberTurn].playerBank);
            displayBorder();
        } else {
            displayBorder();
            System.out.println("\tLetter not found in puzzle or was already guessed!");
            displayBorder();
            game.currentPlayerNumberTurn++;
        }
    ...
    public void buyAVowel(Game game) {        
        ...
        if(count > 0){
            game.playerList[game.currentPlayerNumberTurn].playerBank -= 250;
            displayBorder();
            System.out.println("\tThere are " + count + " " + guessedLetter + "s. Your bank total is now: $" + game.playerList[game.currentPlayerNumberTurn].playerBank);
            displayBorder();
        } else {
